...
$pdf->Cell(0,5,'DATA SURAT MASUK','0','1','C',false);
$pdf->Ln(5);
...
$pdf->Cell(50,8,'PERIHAL',1,0,'C');
$pdf->Cell(50,8,'ASAL SURAT',1,0,'C');
$pdf->Cell(35,8,'NO. SURAT',1,0,'C');
$pdf->Cell(30,8,'TANGGAL SURAT',1,0,'C');
$pdf->Cell(30,8,'PENERIMA',1,1,'C');
$no = 0;

$sql = mysqli_query($connect, "SELECT * FROM surat_masuk, klasifikasi_surat where surat_masuk.id_klasifikasi= klasifikasi_surat.id_klasifikasi");
...
$cellWidth=50; //lebar sel
	...
	if($pdf->GetStringWidth($data['pengirim']) < $cellWidth){
		...
		$textLength=strlen($data['pengirim']);	//total panjang teks
		...
		while($startChar < $textLength){ //perulangan sampai akhir teks
			...
			($startChar+$maxChar) < $textLength ) {
				$maxChar++;
				$tmpString=substr($data['pengirim'],$startChar,$maxChar);
			}
	...
	$pdf->Cell(50,($line * $cellHeight),$data['perihal'],1,0,'C');
	
	$xPos=$pdf->GetX();
	$yPos=$pdf->GetY();
	$pdf->MultiCell($cellWidth,$cellHeight,$data['pengirim'],1,0,'C');
	
	$pdf->SetXY($xPos + $cellWidth , $yPos);
	$pdf->Cell(35,($line * $cellHeight),$data['no_surat'],1,0,'C');
	$pdf->Cell(30,($line * $cellHeight),$data['tanggal_surat'],1,0,'C');
    $pdf->Cell(30,($line * $cellHeight),$data['penerima'],1,1,'C'); 
