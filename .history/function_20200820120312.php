<?php  
	...
	function getDataBuku($jenis = 'semua', $jumlah = false){
		...
		if($jumlah)
			$buku = mysqli_fetch_row(mysqli_query($conn, $query));
		else{
			while(mysqli_fetch_assoc(mysqli_query($conn, $query) as $data)){

			}
		}
		if(empty($buku) && $jumlah)
			return 0;

		return $jumlah ? count($buku) : $buku;
	}
	...
	function pinjambuku($databuku) {
		...
		$jumlahnow = $_GET['jumlah'];
		$jumlah = ($jumlahnow-$databuku);
			
		$update = mysqli_query($conn, "UPDATE tblbuku SET jumlah ='$jumlah' where kode='$kode'" );
