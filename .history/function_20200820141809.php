<?php  
	...
	function kirimpesan($datapesan) {
		...
		// die;
		// Cek Username
		$cekuser = mysqli_fetch_all(mysqli_query($conn, "SELECT username FROM tbluser WHERE username = '$username'"));
		if (count($cekuser) == 0 || empty($cekuser)) {
			echo "
				<script>
			...
			$query = "INSERT INTO tblpesan(`judul`, `isi`, `pengirim`,`username`, `penerima`) VALUES ('$judul','$isi','$pengirim','$username','$penerima')";
