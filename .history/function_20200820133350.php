<?php  
	...
	function buatDropdownBulan($table = 'tblpinjam'){
		global $conn;

		$grouping = $table == 'tblpinjam' ? 'tanggal' : '';
		$result = mysqli_query($conn, "SELECT * FROM $table GROUP BY MONTH($grouping)");
		$dropdown = '<select class="forom-control>"';
		if(!$result)
			echo '<script> alert("Ada kesalahan") </script>';

		while($data = mysqli_fetch_assoc($result)){
		var_dump($data[$grouping]);die;

			$dropdown .= "<option value='$data[$grouping]'> $data[$grouping]</option>";
		}
		$dropdown .= '</select>';
		return $dropdown;
	}
